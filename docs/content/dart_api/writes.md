...
<Snippet href="/lib/src/snippets/dart_api/dataclass.dart" name="companion-custom" />
...
<Snippet href="/lib/src/snippets/modular/upserts.dart" name="words-table" />

By using a custom upserts, we can insert a new word or increment its `usages`
counter if it already exists:

<Snippet href="/lib/src/snippets/modular/upserts.dart" name="track-word" />
...
!!! note "Unique constraints and conflict targets"
    ...
    <Snippet href="/lib/src/snippets/modular/upserts.dart" name="upsert-target" />
...
includes the latest sqlite on Android, so consider using it if you want to support upserts.
