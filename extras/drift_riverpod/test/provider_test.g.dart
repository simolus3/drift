...
extension on TestDatabase {
  ...
  Selectable<User> userByConstant(int var1) {
  ...
  Selectable<User> userById(int var1) {
    return customSelect('SELECT * FROM users WHERE id = ?1', variables: [
      Variable<int>(var1)
    ], readsFrom: {
      users,
...
extension on DatabaseProvider<TestDatabase> {
  SelectableProvider<List<User>> $allUsers(String _) {
    return queryProviderImpl((ref) => ref.watch(_database).allUsers());
  }

  SelectableProvider<User?> $constantUser(String _) {
    return queryProviderImplSingleOrNull(
        (ref) => ref.watch(_database).userByConstant(
              firstId,
            ));
  }

  _$QueryProviderFamily$userById $userById(Object _) =>
      _$QueryProviderFamily$userById($database: _database);
}

// ignore: subtype_of_sealed_class
final class _$QueryProviderFamily$userById
    extends SelectableSingleProviderFamily<User, (int id,)> {
  _$QueryProviderFamily$userById({required super.$database})
      : super(
            $name: r'userById',
            $create: (ref, args) => ref.watch(_database).userById(
                  args.$1,
                ));
  SelectableProvider<User?> call(
    int id,
  ) =>
      create((id,));
}
