...
void main() {
  ...
  test('subqueries', () async {
    ...
    final query = db.selectOnly(db.categories)
      ...
      ..innerJoin(
        subquery,
        on: subquery.ref(db.todosTable.category).equalsExp(db.categories.id),
      )
  ...
  test('compound statements', () async {
    ...
    query.unionAll(
      db.selectExpressions([const Literal<String>(null), countWithoutCategory]),
    );
  ...
  test('full outer join', () async {
    ...
    final query =
        db
          ...
          ..orderBy([
            OrderingTerm.asc(db.todosTable.id, nulls: NullsOrder.first),
          ]);
