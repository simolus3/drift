...
void main() {
  group('with default options', () {
    _testDateTimes(() => TodoDb(testInMemoryDatabase()), dateTimeAsText: true);
  });
  ...
  group('non-datetime expressions', () {
    ...
    test('subqueries cause updates to stream queries', () async {
      ...
      final stream = StreamQueue(
        (db.selectOnly(db.users)..addColumns([subquery]))
...
void _testDateTimes(TodoDb Function() openDb, {bool dateTimeAsText = false}) {
  ...
  group('DateTime', () {
    ...
    test('extracting values', () {
      ...
      expect(
        eval(expr.modify(DateTimeModifier.days(3)).date),
    ...
    test('modifiers', () {
      ...
      expect(
        eval(expr.modify(DateTimeModifier.days(2))),
        completion(result(DateTime.utc(2022, 07, 07))),
      );
      expect(
        eval(expr.modify(DateTimeModifier.months(-2))),
        completion(result(DateTime.utc(2022, 05, 05))),
      );
      expect(
        eval(expr.modify(DateTimeModifier.years(1))),
        completion(result(DateTime.utc(2023, 07, 05))),
      );

      expect(
        eval(expr.modify(DateTimeModifier.hours(12))),
        completion(result(DateTime.utc(2022, 07, 05, 12))),
      );
      expect(
        eval(expr.modify(DateTimeModifier.minutes(30))),
        completion(result(DateTime.utc(2022, 07, 05, 0, 30))),
      );
      expect(
        eval(expr.modify(DateTimeModifier.seconds(30))),
  ...
  }, skip: sqlite3Version.versionNumber < 3039000 ? 'Requires sqlite 3.39' : null);
}
