...
void main() {
  ...
  group('WINDOW FUNCTION', () {
    ...
    test('does not allow empty Order By', () {
      expect(
        () => WindowFunctionExpression<double>(foo.avg(), orderBy: []),
  ...
  group('WINDOW FUNCTION Frame Spec', () {
    ...
    test('allows reverse boundary of same type', () {
      expect(
        FrameBoundary.range(start: -6, end: -1),
        generates("RANGE BETWEEN 6 PRECEDING AND 1 PRECEDING"),
      );

      expect(
        FrameBoundary.groups(start: 4, end: 1),
    ...
    test(
      ...
      () {
        expect(
          () => FrameBoundary.range(start: 6, end: 0),
          throwsA(isA<AssertionError>()),
        );
        expect(
          () => FrameBoundary.range(start: 6, end: -3),
          throwsA(isA<AssertionError>()),
        );
      },
    );
    ...
    test('Exclude boundary', () {
      expect(
        FrameBoundary.range(exclude: FrameExclude.noOthers),
        generates(
          "RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE NO OTHERS",
        ),
      );
      expect(
        FrameBoundary.rows(exclude: FrameExclude.currentRow),
        generates(
          "ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE CURRENT ROW",
        ),
      );
      expect(
        FrameBoundary.groups(exclude: FrameExclude.group),
        generates(
          "GROUPS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE GROUP",
        ),
      );
      expect(
        FrameBoundary.range(exclude: FrameExclude.ties),
        generates(
