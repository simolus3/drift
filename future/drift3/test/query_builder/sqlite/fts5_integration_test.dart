...
void main() {
  ...
  test('match works on aliased tables in joins', () async {
    ...
    final query = db
        .select(db.config)
        .innerJoin(aliased, on: aliased.rowId.isNotNull())
        .where(
  ...
  test('highlight works on aliased tables', () async {
    ...
    for (final column in [db.email.title, aliased.title]) {
      ...
      final query = (db.selectOnly(db.config).addColumns([highlight]))
          .innerJoin(aliased, on: aliased.rowId.isNotNull())
  ...
  test('match works with multiple fts5 tables in one query', () async {
    ...
    final query = db
        .select(docs)
        .innerJoin(db.email, on: db.email.rowId.isNotNull())
  ...
  test('highlight and snippet reject unknown columns', () {
    ...
    final unknown = SchemaColumn<String>(name: 'other_col', sqlType: .text)
      ..owningResultSet = db.email;
