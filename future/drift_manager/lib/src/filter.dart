...
abstract class _BaseColumnFilters<T extends Object> {
  ...
  Expression<bool> sqlEquals(T? value) {
    if (value == null) {
      ...
      return $composableFilter(const Literal(false));
    } else {
...
>
  ...
  TableColumnWithTypeConverter<CustomType, T> get _typedColumn =>
      column as TableColumnWithTypeConverter<CustomType, T>;
...
extension NumFilters<T extends num> on ColumnFilters<T> {
  ...
  Expression<bool> isGreaterThan(T value) =>
      $composableFilter(column.isGreaterThanValue(value));

  /// Create a filter to check if the column is small than a value
  Expression<bool> isLessThan(T value) =>
      $composableFilter(column.isLessThanValue(value));

  /// Create a filter to check if the column is bigger or equal to a value
  Expression<bool> isGreaterOrEqualTo(T value) =>
      $composableFilter(column.isGreaterOrEqualValue(value));

  /// Create a filter to check if the column is small or equal to a value
  Expression<bool> isLessOrEqualTo(T value) =>
      $composableFilter(column.isLessOrEqualValue(value));
