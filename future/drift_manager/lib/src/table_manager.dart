...
// ignore: depend_on_referenced_packages, implementation_imports
import 'package:drift/src/drift3_preview/src/database/connection_user.dart'
    show
        // ignore: invalid_use_of_internal_member
        InternalConnectionUser;
// ignore: depend_on_referenced_packages, implementation_imports
...
class TableManagerState<
  ...
  $Table extends GeneratedTable<$Dataclass, $Table>,
  $Dataclass extends Object,
...
> {
  ...
  List<$ActiveDataclass> toActiveDataclass(List<ManagerResultRow> items) {
    if ($DataclassWithReferences == $ActiveDataclass) {
      ...
      return items.map((e) => e.readTable(table)).toList()
  ...
  >
  withJoin({
    required GeneratedTable currentTable,
    required GeneratedTable referencedTable,
    required TableColumn currentColumn,
    required TableColumn referencedColumn,
  }) {
  ...
  UpdateStatement<$Dataclass, $Table> buildUpdateStatement() {
    final UpdateStatement<$Dataclass, $Table> updateStatement;
  ...
  Future<bool> exists() async {
    ...
    final existsStatement = db.selectOnly(table)..addColumns([query]);
  ...
  DeleteStatement buildDeleteStatement() {
    final DeleteStatement<$Dataclass, $Table> deleteStatement;
    if (joinBuilders.isEmpty) {
...
>
  ...
  ProcessedTableManager<
    ...
    $CreatePrefetchHooksCallback
  >
  withFields(Iterable<BaseComputedField<Object>> computedFields) {
    final joinBuilders = computedFields
        .map((e) => e.joinBuilders)
        .expand((e) => e)
        .toSet();
    final addedColumns = computedFields.map((e) => e.expression).toSet();
    return ProcessedTableManager(
      $state.copyWith(
  ...
  filter(
    ...
    final combinedFilter = switch (($state.filter, filter)) {
      ...
      (var filter1, var filter2) =>
        combineWith == BooleanOperator.and
            ? (filter1!) & (filter2)
            : (filter1!) | (filter2),
    };
    return ProcessedTableManager(
...
>
  ...
  InsertStatement<$Dataclass, $Table> _createInsertStatement(
    InsertMode? mode,
    UpsertClause<$Dataclass, $Table>? onConflict,
  ) {
    var stmt = InsertStatement($state.db, $state.table);
    if (mode != null) stmt.mode(mode);
    if (onConflict != null) stmt.onConflict(onConflict);
    return stmt;
  }
  ...
  Future<int> create(
    ...
    return _createInsertStatement(
      mode,
      onConflict,
    ).insert(f($state._createCompanionCallback));
  }
  ...
  Future<$Dataclass> createReturning(
    ...
    return _createInsertStatement(
      mode,
      onConflict,
    ).insertReturning(f($state._createCompanionCallback));
  }
  ...
  Future<$Dataclass?> createReturningOrNull(
    ...
    return _createInsertStatement(
      mode,
      onConflict,
    ).insertReturningOrNull(f($state._createCompanionCallback));
  }
  ...
  Future<bool> replace(Insertable<$Dataclass> entity) {
    return $state.db.update($state.table).replace(entity);
  }
  ...
  ComputedField<T> computedField<T extends Object>(
  ...
  ComputedFieldWithConverter<DartType, Sql, $Table>
  computedfieldWithConverter<DartType, Sql extends Object>(
    TableColumnWithTypeConverter<DartType, Sql> Function(
