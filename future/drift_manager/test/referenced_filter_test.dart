...
void main() {
  ...
  setUp(() async {
    ...
    await db.managers.categories.bulkCreate(
      (o) => [
        for (final categoryData in _todoCategoryData)
          o(
    ...
    await db.managers.todosTable.bulkCreate(
      (o) => [
        for (final todoData in _todosData)
          o(
  ...
  test('manager - filter related with regualar id', () async {
    ...
    expect(
      await db.managers.product
          ...
          .get(),
      [products[8], products[2], products[1], products[3]],
    );
    ...
    expect(
      ...
      await db.managers.product
          .filter((f) => f.listings((f) => f.price.isGreaterThan(10.0)))
          .count(distinct: true),
      7,
    );
    ...
    expect(
      await db.managers.department.filter((f) {
        ...
        return f.productRefs(
          (f) => f.listings((f) => f.price.isGreaterThan(100.0)),
        );
      }).count(),
      2,
    );
    expect(
      await db.managers.department.filter((f) {
        ...
        return f.productRefs(
          (f) => f.listings(
            (f) => f.price.isGreaterThan(100.0) | f.store.name("Walmart"),
          ),
        );
      }).count(),
      3,
    ...
    expect(
      await db.managers.product
          .filter((f) {
            ...
            return f.listings(
              (f) => f.store.listings(
                (f) => f.product.department.name("Clothing"),
              ),
            );
          })
          .count(distinct: true),
  ...
  test('manager - filter related with custom type for primary key', () async {
    ...
    expect(
      await db.managers.categories
          .filter(
            (f) => f.todos((f) {
              final q = f.category.todos(
                (f) => f.title.equals("Math Homework"),
              );
              return q;
            }),
          )
  ...
  test('manager - with references tests', () async {
    ...
    for (final i in await db.managers.listing.withReferences().get(
      distinct: true,
    )) {
    ...
    expect(
      listingsWithProducts.entries.fold(0, (i, o) => i + o.value.length),
      20,
    );
...
const _todoCategoryData = [
  (description: "School", priority: Value(CategoryPriority.high), id: 1),
  (description: "Work", priority: Value(CategoryPriority.low), id: 2),
];
...
final _todosData =
    ...
    >[
      ...
      (
        ...
        status: Value(TodoStatus.done),
        targetDate: Value(
          DateTime.now().subtract(Duration(days: 2, seconds: 25)),
        ),
      ),
