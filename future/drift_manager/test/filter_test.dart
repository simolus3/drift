...
void main() {
  ...
  test('manager - query number', () async {
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.aReal.isGreaterThan(3.0))
          .count(),
      1,
    );
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.aReal.isGreaterOrEqualTo(3.0))
          .count(),
      2,
    );
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.aReal.isLessThan(5.0))
          .count(),
      1,
    );
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.aReal.isLessOrEqualTo(5.0))
          .count(),
      2,
    );
  ...
  test('manager - query string', () async {
    ...
    await db.managers.tableWithEveryColumnType.create(
      (o) => o(aText: Value("That homework Done")),
    );
    await db.managers.tableWithEveryColumnType.create(
      (o) => o(
  ...
  test('manager - query int64', () async {
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.anInt64.isGreaterThan(BigInt.from(3.0)))
          .count(),
      1,
    );
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.anInt64.isGreaterOrEqualTo(BigInt.from(3.0)))
          .count(),
      2,
    );
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.anInt64.isLessThan(BigInt.from(5.0)))
          .count(),
      1,
    );
    expect(
      await db.managers.tableWithEveryColumnType
          .filter((f) => f.anInt64.isLessOrEqualTo(BigInt.from(5.0)))
          .count(),
      2,
    );
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter(
            (f) => f.anInt64.not.isBetween(BigInt.from(3.0), BigInt.from(5.0)),
          )
          .count(),
      0,
    );
  ...
  test('manager - query bool', () async {
    await db.managers.users.create(
      (o) => o(
        name: "John Doe",
    ...
    // False
    expect(
      await db.managers.users.filter((f) => f.isAwesome.isFalse()).count(),
      1,
    );
    // True
    expect(
      await db.managers.users.filter((f) => f.isAwesome.isTrue()).count(),
      2,
    );
  });
  ...
  test('manager - query custom column', () async {
    ...
    await db.managers.tableWithEveryColumnType.create(
      (o) => o(aText: Value("Get that math homework done")),
    );
    ...
    expect(
      await db.managers.tableWithEveryColumnType
          .filter(
            (f) => f.anIntEnum.not.isIn([
              TodoStatus.open,
              TodoStatus.workInProgress,
            ]),
          )
          .count(),
      1,
