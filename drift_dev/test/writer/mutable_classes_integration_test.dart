...
class _GeneratesWithoutFinalFields extends Matcher {
  ...
  bool matches(dynamic desc, Map matchState) {
    ...
    for (final definedClass in definedClasses) {
      if (expectedWithoutFinals.contains(definedClass.name2.lexeme)) {
        for (final member in definedClass.members) {
          if (member is FieldDeclaration) {
            if (member.fields.isFinal) {
              matchState['desc'] =
                  'Field ${member.fields.variables.first.name2.lexeme} in '
                  '${definedClass.name2.lexeme} is final.';
              return false;
            }
          } else if (member is ConstructorDeclaration) {
            if (member.constKeyword != null) {
              matchState['desc'] =
                  'Constructor ${member.name2?.lexeme ?? ''} in '
                  '${definedClass.name2.lexeme} is constant.';
              return false;
            }
          }
        }
