...
class $$ActiveSessionsTableFilterComposer
  ...
  $$ActiveSessionsTableFilterComposer(super.$db, super.$table,
      {super.$joinBuilder});
  i1.$$UsersTableFilterComposer get user {
    final i1.$$UsersTableFilterComposer composer =
        i1.$$UsersTableFilterComposer($db,
  ...
  i0.ColumnFilters<String> get bearerToken => $columnFilter($table.bearerToken);
}

class $$ActiveSessionsTableOrderingComposer
    extends i0.OrderingComposer<i0.GeneratedDatabase, i3.$ActiveSessionsTable> {
  $$ActiveSessionsTableOrderingComposer(super.$db, super.$table,
      {super.$joinBuilder});
  i1.$$UsersTableOrderingComposer get user => i1.$$UsersTableOrderingComposer(
      $db, i5.ReadDatabaseContainer($db).resultSet<i1.$UsersTable>('users'),
      $joinBuilder: $buildJoinForTable(
          getCurrentColumn: (t) => t.user,
          referencedTable:
              i5.ReadDatabaseContainer($db).resultSet<i1.$UsersTable>('users'),
  ...
  i0.ColumnOrderings<String> get bearerToken =>
      $columnOrdering($table.bearerToken);
}
...
class $$ActiveSessionsTableTableManager extends i0.RootTableManager<
  ...
  $$ActiveSessionsTableTableManager(
      ...
      : super(i0.TableManagerState(
            db: db,
            table: table,
            filteringComposer: $$ActiveSessionsTableFilterComposer(db, table),
            orderingComposer: $$ActiveSessionsTableOrderingComposer(db, table),
