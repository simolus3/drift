...
Future<DriftDatabaseImplementation> driftDatabase({
  ...
  while (true) {
    if (IsolateNameServer.lookupPortByName(portName(name)) case final port?) {
      ...
      try {
        ...
        final controlPort =
            IsolateNameServer.lookupPortByName(isolateControlPortName(name));
        if (controlPort == null) {
          continue;
        }
...
void _isolateEntrypoint(_EntrypointMessage message) {
  ...
  if (IsolateNameServer.registerPortWithName(
    ...
    final server = DriftIsolate.inCurrent(
      () async {
        final (session, _) =
            await NativeDatabase.blocking(File(message.path)).open();
        return session;
      },
      port: connections,
      beforeShutdown: () {
